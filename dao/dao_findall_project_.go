...
func DB_FindallProject(page, size, searchTerm string, noPagination bool) (int64, *[]dto.Project, error) {
	var skip int64 = 0
	var limit int64 = 0
	...
	if searchTerm != "" {
		searchConditions := []bson.M{
			...
			{"startdate": bson.M{"$regex": searchTerm, "$options": "i"}},
			{"enddate": bson.M{"$regex": searchTerm, "$options": "i"}},
			{"status": bson.M{"$regex": searchTerm, "$options": "i"}},
			{"workspaceid": bson.M{"$regex": searchTerm, "$options": "i"}},
		}
	...
	cursor, err := dbConfig.DATABASE.Collection("Projects").Aggregate(ctx, pipeline)
	if err != nil {
		return 0, nil, err
	}
