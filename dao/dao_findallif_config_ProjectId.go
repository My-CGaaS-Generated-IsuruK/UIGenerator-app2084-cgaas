...
func DB_FindallifConfigbyProjectId (ProjectId string, page, size, searchTerm string, noPagination bool) (int64, *[]dto.Config, error) {
    var skip int64 = 0
    var limit int64 = 0
    ...
    if searchTerm != "" {
        searchConditions := []bson.M{
			{"configid": bson.M{"$regex": searchTerm, "$options": "i"}},
			{"value": bson.M{"$regex": searchTerm, "$options": "i"}},
    ...
    cursor, err := dbConfig.DATABASE.Collection("Configs").Aggregate(ctx, pipeline)
    if err != nil {
        return 0, nil, err
    }
