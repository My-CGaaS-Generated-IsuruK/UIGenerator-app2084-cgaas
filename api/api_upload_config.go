...
// @Description   This API performs the UPLOAD operation on Config. It allows you to  Config records.
// @Tags          Config
// @Accept       json
// @Produce      json
...
// @Router      /UploadConfig [UPLOAD]

    func UploadConfigApi(c *fiber.Ctx) error {
	...
	for {
		...
		var ConfigStruct dto.Config
		if err := json.Unmarshal(objectJSON, &ConfigStruct); err != nil {
			fmt.Println("Error unmarshalling object JSON:", err)
			return utils.SendErrorResponse(c, fiber.StatusInternalServerError, err.Error())
		}

		ConfigStruct.ConfigId = uuid.New().String()


		err = dao.DB_CreateConfig(&ConfigStruct)
		if err != nil {
			return utils.SendErrorResponse(c, fiber.StatusInternalServerError, err.Error())
		}
	}
