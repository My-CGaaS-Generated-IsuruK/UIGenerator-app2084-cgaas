package api

import (
  
"UIGenerator/utils"
"github.com/gofiber/fiber/v2"

  
    "UIGenerator/dao"
    
  

  
  
  
)

// @Summary      FindallProject 
// @Description   This API performs the GET operation on Project. It allows you to retrieve Project records.
// @Tags          Project
// @Accept       json
// @Produce      json
// @Param        objectId query []string false "string collection"  collectionFormat(multi)
// @Success      200  {array}   dto.Project "Status OK"
// @Success      202  {array}   dto.Project "Status Accepted"
// @Failure      404 "Not Found"
// @Router      /FindallProject [GET]

    func FindallProjectApi(c *fiber.Ctx) error {





    
            page := c.Query("page", "1")
                size := c.Query("size", "10")
                searchTerm := c.Query("searchTerm", "")
                noPaginationStr := c.Query("noPagination", "")
                noPagination := noPaginationStr == "true"
            
        



    
  Count, Project ,err := dao.DB_FindallProject(page,size,searchTerm,noPagination)
    if err != nil {
        return utils.SendErrorResponse(c, fiber.StatusBadRequest, err.Error())
    }    


returnValue := map[string]interface{}{
                "Count":    Count,
                "Project": Project,
            }
    
        return c.Status(fiber.StatusAccepted).JSON(&returnValue)
}

